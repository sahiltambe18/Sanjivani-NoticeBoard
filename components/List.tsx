...
export default function List() {
  ...
  return (
    <ul ref={listRef} className='w-full  flex flex-col items-start text-[#fffef8] overflow-y-auto sm:pl-10' style={{ maxHeight: '80vh' }}>
      {data && data.map((notice, i) => (
        <div className='my-4' key={notice.id}>
          ...
          {notice.imageUrl && <img className='h-72 rounded-lg border-2 border-gray-200' src={notice.imageUrl} alt={notice.title} />}
        </div>
      ))}
    </ul>
  )
